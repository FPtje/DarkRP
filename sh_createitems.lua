...
local function addTeamCommands(CTeam, max)
    ...
    if CTeam.vote or CTeam.RequiresVote then
        ...
        local function onJobCommand(ply, hasPriv)
            ...
            if a > 0 and not ply:IsAdmin() or a > 1 and not ply:IsSuperAdmin() then
                DarkRP.notify(ply, 1, 4, DarkRP.getPhrase("need_admin", CTeam.name))

                return
            end

            if not CTeam.RequiresVote and (a == 0 and not ply:IsAdmin() or a == 1 and not ply:IsSuperAdmin() or a == 2) or CTeam.RequiresVote and CTeam.RequiresVote(ply, k) then
...
local function addEntityCommands(tblEnt)
    ...
    local function defaultSpawn(ply, tr, tblE)
        ...
        if not ent:IsValid() then
            error("Entity '" .. tblE.ent .. "' does not exist or is not valid.")
        end
    ...
    local function buythis(ply, args)
        ...
        if canbuy == false then
            if not suppress and message then
                DarkRP.notify(ply, 1, 4, message)
            end
...
function DarkRP.createJob(Name, colorOrTable, model, Description, Weapons, command, maximum_amount_of_this_class, Salary, admin, Vote, Haslicense, NeedToChangeFrom, CustomCheck)
    ...
    if not valid then
        DarkRP.error(string.format("Corrupt team: %s!\n%s", CustomTeam.name or "", err), 2, hints)
    end
...
DarkRP.createJob = DarkRP.createJob
...
local function removeCustomItem(tbl, category, hookName, reloadF4, i)
    ...
    if category then
        DarkRP.removeFromCategory(item, category)
    end

    if istable(item) and (item.command or item.cmd) then
        DarkRP.removeChatCommand(item.command or item.cmd)
    end

    hook.Run(hookName, i, item)

    if CLIENT and reloadF4 and IsValid(DarkRP.getF4MenuPanel()) then
...
function DarkRP.createShipment(name, model, entity, price, Amount_of_guns_in_one_shipment, Sold_separately, price_separately, noshipment, classes, shipmodel, CustomCheck)
    ...
    local customShipment = tableSyntaxUsed and model or {
        ...
        seperate = Sold_separately,
        pricesep = price_separately,
        noship = noshipment,
        allowed = classes,
    ...
    if not valid then
        DarkRP.error(string.format("Corrupt shipment: %s!\n%s", name or "", err), 2, hints)
    end
    ...
    if not customShipment.noship then
        DarkRP.addToCategory(customShipment, "shipments", customShipment.category)
    end

    if customShipment.separate then
        DarkRP.addToCategory(customShipment, "weapons", customShipment.category)
    end
...
DarkRP.createShipment = DarkRP.createShipment
...
function DarkRP.createVehicle(Name_of_vehicle, model, price, Jobs_that_can_buy_it, customcheck)
    ...
    for k, v in pairs(DarkRP.getAvailableVehicles()) do
        if string.lower(k) == string.lower(vehicle.name) then
            found = true
            break
        end
    end
    ...
    if not valid then
        DarkRP.error(string.format("Corrupt vehicle: %s!\n%s", vehicle.name or "", err), 2, hints)
    end

    if not found then
        DarkRP.error("Vehicle invalid: " .. vehicle.name .. ". Unknown vehicle name.", 2)
    end
...
DarkRP.createVehicle = DarkRP.createVehicle
...
function DarkRP.createEntity(name, entity, model, price, max, command, classes, CustomCheck)
    ...
    if not valid then
        DarkRP.error(string.format("Corrupt entity: %s!\n%s", name or "", err), 2, hints)
    end
    ...
    timer.Simple(0, function()
        addEntityCommands(tblEnt)
    end)
end

DarkRP.createEntity = DarkRP.createEntity
...
function DarkRP.createAgenda(Title, Manager, Listeners)
    ...
    if not valid then
        DarkRP.error(string.format("Corrupt agenda: %s!\n%s", agenda.Title or "", err), 2, hints)
    end
...
GM.AddAmmoType = function(_, ...)
    DarkRP.createAmmoType(...)
end
...
function DarkRP.createDemoteGroup(name, tbl)
    ...
    if not tbl or not tbl[1] then
        error("No members in the demote group!")
    end
...
function DarkRP.createCategory(tbl)
    ...
    if not valid then
        DarkRP.error(string.format("Corrupt category: %s!\n%s", tbl.name or "", err), 2, hints)
    end
...
function DarkRP.addToCategory(item, kind, cat)
    ...
    DarkRP.errorNoHalt(string.format([[The category of "%s" ("%s") does not exist!]], item.name, cat), 2, {"Make sure the category is created with DarkRP.createCategory.", "The category name is case sensitive!", "Categories must be created before DarkRP finished loading."})
end
...
function DarkRP.removeFromCategory(item, kind)
    ...
    if not cats then
        DarkRP.error(string.format("Invalid category kind '%s'.", kind), 2)
    end
...
local function mergeCategories(customs, catKind, path)
    ...
    for k, v in pairs(customs) do
        ...
        if not cat then
            DarkRP.errorNoHalt(string.format([[The category of "%s" ("%s") does not exist!]], v.name, catName), 3, {"Make sure the category is created with DarkRP.createCategory.", "The category name is case sensitive!", "Categories must be created before DarkRP finished loading."}, path, -1, path)
            cat = catByName.Other
        end
    ...
    for k, v in pairs(cats) do
        table.sort(v.members, categoryOrder)
    end
end
