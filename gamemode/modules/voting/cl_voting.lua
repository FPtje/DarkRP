...
local function MsgDoVote(msg)
	...
	panel:SetTitle(fprp.getPhrase("vote"))
	...
	label:SetPos(5, 25)
	label:SetText(fprp.deLocalise(question))
	...
	ybutton:SetCommand("!")
	ybutton:SetText(fprp.getPhrase("yes"))
	...
	nbutton:SetCommand("!")
	nbutton:SetText(fprp.getPhrase("no"))
	...
	panel:SetSkin(GAMEMODE.Config.fprpSkin)
end
...
local function MsgDoQuestion(msg)
	...
	function panel:Think()
		self:SetTitle(fprp.getPhrase("time", math.Clamp(math.ceil(timeleft - (CurTime() - OldTime)), 0, 9999)))
		if timeleft - (CurTime() - OldTime) <= 0 then
			panel:Close()
		end
	end
