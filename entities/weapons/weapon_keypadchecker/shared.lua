if SERVER then
	...
	util.AddNetworkString("fprp_keypadData");
end
...
local function getTargets(keypad, keyPass, keyDenied, delayPass, delayDenied)
	...
	for k,v in pairs(numpad.OnDownItems or {}) do
		if v.key == keyPass and v.ply == Owner then
			table.insert(targets, {type = fprp.getPhrase("keypad_checker_entering_right_pass"), name = v.name, ent = v.ent, original = keypad});
		end
		if v.key == keyDenied and v.ply == Owner then
			table.insert(targets, {type = fprp.getPhrase("keypad_checker_entering_wrong_pass"), name = v.name, ent = v.ent, original = keypad});
		end
	end

	for k,v in pairs(numpad.OnUpItems or {}) do
		if v.key == keyPass and v.ply == Owner then
			table.insert(targets, {type = fprp.getPhrase("keypad_checker_after_right_pass"), name = v.name, delay = math.Round(delayPass, 2), ent = v.ent, original = keypad});
		end
		if v.key == keyDenied and v.ply == Owner then
			table.insert(targets, {type = fprp.getPhrase("keypad_checker_after_wrong_pass"), name = v.name, delay = math.Round(delayDenied, 2), ent = v.ent, original = keypad});
		end
	end
...
local function getEntityKeypad(ent)
	...
	for k,v in pairs(ents.FindByClass("sent_keypad")) do
		...
		if vOwner == entOwner and table.HasValue(doorKeys, v:GetNWInt("keypad_keygroup1")) then
			table.insert(targets, {type = fprp.getPhrase("keypad_checker_right_pass_entered"), ent = v, original = ent});
		end
		if vOwner == entOwner and  table.HasValue(doorKeys, v:GetNWInt("keypad_keygroup2")) then
			table.insert(targets, {type = fprp.getPhrase("keypad_checker_wrong_pass_entered"), ent = v, original = ent});
		end
	end
...
function SWEP:PrimaryAttack()
	...
	if class == "sent_keypad" then
		...
		fprp.notify(self.Owner, 1, 4, fprp.getPhrase("keypad_checker_controls_x_entities", #data / 2));
