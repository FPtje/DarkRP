    ...
    if ent:IsPlayer() or ent:IsNPC() or ent:IsVehicle() then
        ...
        if FPP and FPP.plyCanTouchEnt(self:GetOwner(), ent, "EntityDamage") then
            if ent.SeizeReward and not ent.beenSeized and not ent.burningup and self:GetOwner():isCP() and ent.Getowning_ent and self:GetOwner() ~= ent:Getowning_ent() then
                local money = isfunction(ent.SeizeReward) and ent:SeizeReward(self:GetOwner(), dmg) or ent.SeizeReward
                self:GetOwner():addMoney(money)
                DarkRP.notify(self:GetOwner(), 1, 4, DarkRP.getPhrase("you_received_x", DarkRP.formatMoney(money), DarkRP.getPhrase("bonus_destroying_entity")))
                ent.beenSeized = true
            end
