...
function ENT:Draw()
    ...
    local owner = (IsValid(self:Getowning_ent()) and self:Getowning_ent().Name and self:Getowning_ent():Nick()) or DarkRP.getPhrase("unknown")
    local recipient = (self:Getrecipient().Name and self:Getrecipient():Nick()) or DarkRP.getPhrase("unknown")
    ...
    if recipient == LocalPlayer():Nick() and owner ~= LocalPlayer():Nick() then
        draw.DrawNonParsedText(text, "ChatFont", -TextWidth * 0.5, -25, Color(0, 255, 0, 255), 0)
    elseif recipient == LocalPlayer():Nick() and owner == LocalPlayer():Nick() then
