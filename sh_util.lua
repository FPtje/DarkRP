...
function DarkRP.findPlayer(info)
    ...
    for k = 1, #pls do
        ...
        if string.find(string.lower(v:SteamName()), string.lower(tostring(info)), 1, true) ~= nil then return v end
    end
...
function DarkRP.findPlayers(info)
    ...
    for _, PlayerInfo in pairs(InfoPlayers) do
        ...
        for k, v in pairs(pls) do
            ...
            if (PlayerInfo == v:SteamID() or v:SteamID() == "UNKNOWN") or string.find(string.lower(v:Name()), string.lower(tostring(PlayerInfo)), 1, true) ~= nil or (v.SteamName and string.find(string.lower(v:SteamName()), string.lower(tostring(PlayerInfo)), 1, true) ~= nil) then
                found[v] = true
                players = players or {}
...
function meta:getEyeSightHitEntity(searchDistance, hitDistance, filter)
    ...
    if smallestDistance < hitDistance then return foundEnt, math.sqrt(smallestDistance) end
...
function meta:hasDarkRPPrivilege(priv)
    if FAdmin then return FAdmin.Access.PlayerHasPrivilege(self, priv) end
...
function DarkRP.explodeArg(arg)
    ...
    if from ~= to + 1 then
        tableinsert(args, stringsub(arg, from + diff, to + 1 - bitlshift(diff, 1)))
    end
